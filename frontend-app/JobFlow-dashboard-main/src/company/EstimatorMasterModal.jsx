            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="relative">
                <textarea
                 ...
                 value={`<!-- CoatingPro360 Estimator Form --><iframe id="cp360-estimator" src="https://estimate.coatingpro360.com/?company=${company.estimatorCode}" width="100%" height="900" style="border:0; border-radius:14px;" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe><script>(function(){function a(){try{var u=['utm_source','utm_medium','utm_campaign'],p=new URLSearchParams(window.location.search),f=document.getElementById('cp360-estimator')||document.querySelector('iframe[src*="estimate.coatingpro360.com"]');if(!f)return;var s=new URL(f.src);u.forEach(function(k){if(p.get(k))s.searchParams.set(k,p.get(k));});f.src=s.toString();}catch(e){}}document.readyState==='loading'?document.addEventListener('DOMContentLoaded',a):a();})();window.addEventListener('message',function(e){if(!e.data||e.data.event!=='estimator_conversion')return;if(e.data.microsoftEventCode){try{new Function(e.data.microsoftEventCode)();}catch(err){}}});<\/script><!-- / CoatingPro360 Estimator Form -->`}
                />
