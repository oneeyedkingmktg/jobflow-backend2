...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, lead_id, scheduled_date, scheduled_end_date, scheduled_time, scheduled_end_time, sc_type, title, notes, ghl_event_id, created_at, updated_at
       FROM service_calls
       WHERE lead_id = $1
...
router.post("/", async (req, res) => {
  try {
    const { leadId } = req.params;
    const companyId = req.user.company_id;
    const { scheduled_date, scheduled_end_date, scheduled_time, scheduled_end_time, sc_type, title, notes } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO service_calls (lead_id, company_id, scheduled_date, scheduled_end_date, scheduled_time, scheduled_end_time, sc_type, title, notes)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, lead_id, scheduled_date, scheduled_end_date, scheduled_time, scheduled_end_time, sc_type, title, notes, ghl_event_id, created_at, updated_at`,
      [leadId, leadCompanyId, scheduled_date || null, scheduled_end_date || null, scheduled_time || null, scheduled_end_time || null, sc_type || 'service_call', title || null, notes || null]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE service_calls
       SET scheduled_date = $1, scheduled_end_date = $2, scheduled_time = $3, scheduled_end_time = $4, sc_type = $5, title = $6, notes = $7, updated_at = NOW()
       WHERE id = $8
       RETURNING id, lead_id, scheduled_date, scheduled_end_date, scheduled_time, scheduled_end_time, sc_type, title, notes, ghl_event_id, created_at, updated_at`,
      [scheduled_date || null, scheduled_end_date || null, scheduled_time || null, scheduled_end_time || null, sc_type || 'service_call', title || null, notes || null, id]
    );
