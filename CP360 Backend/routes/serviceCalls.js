...
router.get("/", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, lead_id, scheduled_date, scheduled_time, title, notes, ghl_event_id, created_at, updated_at
       FROM service_calls
       WHERE lead_id = $1
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO service_calls (lead_id, company_id, scheduled_date, scheduled_time, title, notes)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, lead_id, scheduled_date, scheduled_time, title, notes, ghl_event_id, created_at, updated_at`,
      [leadId, leadCompanyId, scheduled_date || null, scheduled_time || null, title || null, notes || null]
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE service_calls
       ...
       WHERE id = $5
       RETURNING id, lead_id, scheduled_date, scheduled_time, title, notes, ghl_event_id, created_at, updated_at`,
      [scheduled_date || null, scheduled_time || null, title || null, notes || null, id]
    );
