  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO bidder_company_settings (company_id, default_payment_url, include_payment_button, down_payment_default_percent, preferred_proposal_design_id, terms_and_conditions, system_notes, email_from_name, email_from_email, proposal_top_text, invoice_top_text, updated_at)
       ...
       RETURNING *`,
      [
        companyId, clean(default_payment_url), include_payment_button,
...
// GET /api/bidder/proposal-designs — all companies can read (to pick a design)
router.get('/proposal-designs', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM bidder_proposal_designs WHERE is_active = true ORDER BY name'
    );
    res.json(result.rows);
  } catch (err) {
