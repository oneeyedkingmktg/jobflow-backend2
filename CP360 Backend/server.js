...
async function runMigrations() {
  const migrations = [
    ...
    `INSERT INTO report_definitions (key, name, description) VALUES
      ('automation_recovery', 'Automation Recovery', 'Full funnel closings (lead → sold) and Not Sold recoveries filtered by sold date. Shows your marketing system is working.')
    ...
    `ALTER TABLE crews ADD COLUMN IF NOT EXISTS has_calendar BOOLEAN DEFAULT false`,
    ...
    `ALTER TABLE time_entries ALTER COLUMN work_type SET DEFAULT 'job'`,
    `ALTER TABLE time_entries ADD COLUMN IF NOT EXISTS clock_in TIMESTAMPTZ DEFAULT NOW()`,
    `ALTER TABLE time_entries ALTER COLUMN clock_in SET DEFAULT NOW()`,
