...
// Uses DELETE /calendars/events/appointments/{eventId} to permanently remove
// the event from GHL. Applies to appointment, install, and service call
// calendar events across all three calendars.
// ----------------------------------------------------------------------------
async function deleteCalendarEvent(company, eventId, contactId) {
  ...
  try {
    ...
    console.warn("⚠️ [CALENDAR DELETE] GHL rejected delete:", err.message);
    return false;
  }
}
...
async function syncServiceCallToGhl({ serviceCall, lead, company }) {
  ...
  // appointmentStatus: "confirmed" bypasses GHL's slot availability check.
  // ignoreDateRanges only bypasses date-range restrictions, not slot/time validation.
  const createPayload = {
    locationId: company.ghl_location_id,
    calendarId,
    contactId: lead.ghl_contact_id,
  ...
  console.log("[SC GHL] Creating appointment event:", JSON.stringify(createPayload));
  const created = await ghlRequest(company, '/calendars/events/appointments', {
    method: 'POST',
    body: createPayload,
  });
