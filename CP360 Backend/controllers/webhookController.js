handleGHLContact: async (req, res) => {
  ...
  try {
    ...
    if (existingLead) {
      // ⏱️ DEDUPLICATION: Check if recently synced (within 2 minutes)
      ...
      updateValues.push(existingLead.id); // WHERE clause parameter
      
      // 🔧 FIX: Actually execute the UPDATE query!
